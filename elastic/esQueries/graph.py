...
file_path = 'adjustedUpgradeResults.json'
data = pd.read_json(file_path)

# Extract relevant data
buckets = data['failed_upgrade_jobs_by_half_hour']['buckets']
...
ax1.set_xticklabels(time_frames[::step], rotation=0, ha='center')

fig.tight_layout()
ax1.legend(loc='upper left')
ax2.legend(loc='upper right')
plt.title('Upgrade Statistics by Time Frame')
plt.show()
