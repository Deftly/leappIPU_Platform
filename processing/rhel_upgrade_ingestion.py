...
def scrape(baseurl, endpoint, query, auth, data):
    ...
    try:    
        response = requests.get('{}{}{}'.format(baseurl, endpoint, query),
                                cookies=auth, verify=False)
        tmp = response.json()
        if "results" in tmp:
...
def get_playbooks(region, start_time, auth):
    ...
    query = ('?format=json&name__icontains=leapp&not__finished__isnull=true'
             '&type=job'
...
def generate_workflows(playbooks, region, auth, ids):
    ...
    for playbook in playbooks:
        playbook['created'] = datetime.strptime(playbook['created'],
                                                         "%Y-%m-%dT%H:%M:%S.%f%z")
        playbook['started'] = datetime.strptime(playbook['started'],
                                                         "%Y-%m-%dT%H:%M:%S.%f%z")
        playbook['finished'] = datetime.strptime(playbook['finished'],
                                                          "%Y-%m-%dT%H:%M:%S.%f%z")
        ...
        if play_id not in ids:
            ...
            if (playbook['automation_failure'] 
                and not any([i['automation_failure'] for i in playbook['failed_tasks']])):
...
def validate_workflows(workflows, region, latest_job):
    ...
    for workflow in workflows:
        ...
        if (workflow_automation_failure 
            and not any([i['automation_failure'] for i in jobs])):
        ...
        if workflow_type == 'operational_check_7_to_8':
            if len(jobs) > 1:
                print(("Invalid Operational Check 7 to 8 Workflow: "
            ...
            if len(jobs) > 1:
                print(("Invalid Operational Check 7 to 9 Workflow: "
                       f"{workflow_type} {jobs[0]['id']} {jobs[0]['extra_vars']['txId']}"))
                print([i['id'] for i in jobs])
                failed = True
            pass
            ...
            if len(jobs) > 1:
                print(("Invalid Operational Check 8 to 9 Workflow: "
                       f"{workflow_type} {jobs[0]['id']} {jobs[0]['extra_vars']['txId']}"))
                print([i['id'] for i in jobs])
                failed = True
            pass
        elif workflow_type == 'inhibitor_check_7_to_8':
            if len(jobs) == 1 and ('operational' in jobs[0]['name']) and workflow_failed:
                pass
            elif len(jobs) == 3 and ('rollback' in jobs[-1]['extra_vars']['changefile_tasks_from']):
                pass
            else:
                print(("Invalid Inhibitor Check 7 to 8 Workflow: "
                      f"{workflow_type} {jobs[0]['id']} {jobs[0]['extra_vars']['txId']}"))
                print([i['id'] for i in jobs])
                failed = True
            ...
            if len(jobs) == 1 and ('operational' in jobs[0]['name']) and workflow_failed:
                ...
                print(("Invalid Inhibitor Check 8 to 9 Workflow: "
                      f"{workflow_type} {jobs[0]['id']} {jobs[0]['extra_vars']['txId']}"))
                print([i['id'] for i in jobs])
                failed = True
        elif workflow_type == 'upgrade_7_to_8':
            if workflow_failed and ('vastool_revert' in jobs[-1]['extra_vars']['sub_workflow']):
                pass
            elif ((not workflow_failed) and ("changefile_included_role" in jobs[-1]['extra_vars'].keys()) and
                 ('postupgrade_7_to_8' in jobs[-1]['extra_vars']['changefile_included_role'])):
                pass
            else:
                print(("Invalid Invalid or Incomplete Upgrade 7 to 8 Workflow: "
                      f"{workflow_type} {jobs[0]['id']} {jobs[0]['extra_vars']['txId']}"))
                print([i['id'] for i in jobs])
                failed = True
            ...
            if workflow_failed and ('vastool_revert' in jobs[-1]['extra_vars']['sub_workflow']):
                pass
                 ...
                 ('postupgrade_8_to_9' in jobs[-1]['extra_vars']['changefile_included_role'])):
                pass
            else:
                print(("Invalid Invalid or Incomplete Upgrade 8 to 9 Workflow: "
                      f"{workflow_type} {jobs[0]['id']} {jobs[0]['extra_vars']['txId']}"))
            ...
            if workflow_failed and ('vastool_revert' in jobs[-1]['extra_vars']['sub_workflow']):
                pass
            elif (not workflow_failed) and ("changefile_included_role" in jobs[-1]['extra_vars']) and \
                 ('postupgrade_8_to_9' in jobs[-1]['extra_vars']['changefile_included_role']):
                pass
            else:
                print(("Invalid Invalid or Incomplete Upgrade 7 to 9 Workflow: "
                      f"{workflow_type} {jobs[0]['id']} {jobs[0]['extra_vars']['txId']}"))
                print([i['id'] for i in jobs])
                failed = True
        else:
        ...
        tentative_workflow = {
            ...
            "workflow_done": (not failed) or 
                             (latest_job.astimezone(tz=pytz.utc) - jobs[-1]['finished'] > timedelta(hours=8))
        }
...
def format_failed_task(failed_task):
    ...
    if 'event_data' in failed_task:
        for ed_key in set(failed_task['event_data'].keys()) - _FAILED_TASKS_EVENT_DATA_KEYS:
...
def gather_region_data(region, cookie):
    ...
    if len(ids) >= 9000:
        raise Exception(("Too many workflows have been run in the last 24 hours."
